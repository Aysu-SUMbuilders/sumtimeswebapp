...
from my_WEB import app, db, mail, Message
from my_WEB.forms import RegisterForm, LoginForm, PostForm, ContactForm
from my_WEB.models import User, Post
...
def contact():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            msg = Message(form.name.data, sender ='SUMbuilders Contact Form', recipients = ['tural.suleymanov.8691@gmail.com']) 
            msg.body = """
            From: %s
            <%s>
            %s
            ...
            flash(f'OOPS There is a problem', 'danger')
    elif request.method == 'GET':
